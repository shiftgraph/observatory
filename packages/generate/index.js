...
/**
 * A fixture value for a profile: the right shape, with placeholder values.
 *
 * It carries no real data and cannot, because the record holds no values. That
 * is the point rather than a limitation. A fixture built from a captured
 * response embeds whatever happened to be in it, which is how a test suite ends
 * up asserting against one customer's name; a fixture built from a contract
 * asserts against the contract, which is the only thing a fixture should ever
 * have been asserting against.
 *
 * Optional fields are INCLUDED. A fixture is the shape a consumer must handle,
 * and omitting the optional half would let a test pass against a response the
 * interface is entitled to send.
 */
export function fixtureValue(node, depth = 0) {
  if (!node || typeof node !== 'object') return null;
  if (depth > 12) return null;
  switch (node.type) {
    case 'string':
      return node.format === 'date-time' ? '1970-01-01T00:00:00.000Z' : 'string';
    case 'number':
      return 0;
    case 'integer':
      return 0;
    ...
    case 'null':
      return null;
    case 'array':
      // One element, so a consumer that maps over it runs its body exactly
      // once. Zero would skip the body and the test would assert nothing.
      return node.element ? [fixtureValue(node.element, depth + 1)] : [];
    case 'object': {
      const keys = node.keys || {};
      const out = {};
      for (const k of Object.keys(keys)) {
        // A map keyed by data: give it one representative entry rather than a
        // key called "{key}", which is not a key any response would carry.
        out[k === '{key}' ? 'key' : k] = fixtureValue(keys[k], depth + 1);
      }
      return out;
    }
    default:
...
/**
 * A committable fixture module: the value, typed against the generated type.
 *
 * Typed on purpose. An untyped fixture drifts from the type beside it in
 * silence, which is exactly the failure this tool exists to remove, one level
 * in. Annotated, a regenerated type that no longer matches the fixture is a
 * compile error rather than a test that quietly keeps passing.
 */
export function generateFixture({ name, profile, source, observations, observedFrom, observedTo, command, typeModule = null }) {
  const typeName = pascal(name);
  const value = JSON.stringify(fixtureValue(profile), null, 2);
  const importFrom = typeModule || `./${slugForFixture(name)}`;
  return [
    '// Generated by ShiftGraph from OBSERVED responses. Do not edit by hand.',
    '//',
    `// source        : ${source}`,
    ...
    `// observed      : ${observedFrom}${observedTo && observedTo !== observedFrom ? ` to ${observedTo}` : ''}`,
    '//',
    '// Structure only. Values are placeholders, because the record holds no',
    '// values: a fixture built from a captured response embeds whatever happened',
    '// to be in it, and a test asserting against that is asserting against one',
    '// response rather than against the contract.',
    '//',
    '// Optional fields are present. A fixture is the shape your code must',
    '// handle, and a fixture missing the optional half lets a test pass against',
    '// a response the interface is entitled to send.',
    '//',
    `// Regenerate: ${command}`,
    '',
    `import type { ${typeName} } from '${importFrom}';`,
    '',
    `export const ${camel(name)}Fixture: ${typeName} = ${value};`,
    '',
  ].join('\n');
}

function slugForFixture(name) {
  return String(name).toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '') || 'contract';
}
