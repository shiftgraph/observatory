...
Four stand in the published window as of 19 August 2026, all at low severity, and they are **not** all the same kind. One is incident-state variation in a status feed — a field a status page carries while an incident is in the monitoring state and not otherwise — on `status.plaid.com`. The other three are on `formulae.brew.sh` and they are the real thing: Homebrew added a `vulnerabilities` object to the formula API, a second formula moved from 116 observed fields to 137 with an array element promoted from empty to a typed object, and a cask gained a `supported_platforms` array. Each was checked against the committed contract records in `data/contracts/` rather than accepted from the run that raised it, and they arrived on separate sweeps eight days apart, so this is not one event counted three times.

**So the earlier claim that none of these is a provider altering an interface is no longer true, and that is the more interesting half.** Three of the four are exactly that: a third party changed a published interface and this record caught it. All three are **additive** — a consumer reading the old fields is unaffected — and no breaking change has ever been caught here. The distinction between an additive change and a breaking one is why severity is a column rather than a headline.

**Two claims that stood here are gone and should not be re-added.** `api.coinbase.com/v2/exchange-rates` was retracted on 2026-08-04, `www.planetscalestatus.com` and `hex.pm` on 2026-08-19: each was our own profiler rather than the provider. Every row stays in `history.ndjson` where anyone can read what was claimed and when.
